        ...
        <div
            dangerouslySetInnerHTML={{
                __html: `<a href='${link}' class='inline-flex items-center text-sm md:text-base leading-none w-max px-3 md:px-0 h-[40px] md:w-[200px] md:h-[50px] rounded-xl ${css} justify-center gap-2 uppercase transition-all duration-300'>
                    ${icon === "chat" ?
                        `<img loading="lazy" class='transition-all duration-300' src="/icons/chat.png" width="16" height="16" />` :
                        ``}
                    ${icon === "chat-pink" ?
                        `<img loading="lazy" class='transition-all duration-300' src="/icons/chat-pink.png" width="17" height="17" />` :
                        ``}
                    ${icon === "call" ?
                        `<img loading="lazy" class='transition-all duration-300' src="/icons/call-pink.png" width="17" height="16" />` :
                        ``}
                    ${icon === "call-pink" ?
                        `<img loading="lazy" class='transition-all duration-300' src="/icons/call-pink.png" width="17" height="16" />` :
                        ``}
...
export function CTAGroupLight() {
    return (
        <>
            <CTA />
            <CTA icon="call-pink" css="bg-transparent border-2 text-pink border-pink" text="(833) 0001 - 2222" />
        </>
    )
}
